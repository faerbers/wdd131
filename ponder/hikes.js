...
//target elements needed to manipulate/listen for an event in JS
...
function search() {
    ...
    //now organize search results by difficulty
    ...
    //clear out extra so nothing matching will be in results
...
//Display a random hike to begin with and output to screen
// in this case it's 0-4
let randomNum = Math.floor(Math.random() * hikes.length)
console.log(randomNum);

//helps add tags whether they have 1 or more
function tagTemplate(tags) {
    return tags.map((tag)=> `<button>${tag}</button>`).join(' ');
}

//adds difficulty 1 out of 5 or 3 out of 5, and so on
...
//as soon as page opens it runs init
init();